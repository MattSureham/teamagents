...
export const createMeetingSchema = z.object({
  ...
  speakerOrder: z.array(z.string()).optional().describe('Optional speaking order; IDs not listed are appended after the listed agents'),
...
export const resumeMeetingSchema = z.object({
  ...
  speakerOrder: z.array(z.string()).optional().describe('Override speaking order'),
  turnTimeoutMs: z.number().int().min(1000).optional(),
  maxRebuttalRounds: z.number().int().min(0).max(20).optional(),
  maxDeliberationRounds: z.number().int().min(0).max(20).optional(),
  maxPlanRounds: z.number().int().min(0).max(20).optional(),
